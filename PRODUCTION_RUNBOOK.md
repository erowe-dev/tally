...
- Root fallback: committed `vercel.json`, `tally/vercel.json`, and `tally/landing/vercel.json` mirror routing/header rules so Vercel serves the prepared `browser` output from the configured project root.
...
2. Confirm project `tally` has Output Directory `browser`; if Root Directory is `tally/landing`, keep `tally/landing/vercel.json` in sync with the app/root Vercel configs.
