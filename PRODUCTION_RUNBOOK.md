...
- Root fallback: committed `vercel.json` mirrors `tally/vercel.json` so Vercel builds from the configured project root and serves `dist/tally/browser`.
...
2. Confirm project `tally` has Root Directory `tally`, Build Command `npm run build`, and Output Directory `dist/tally/browser`.
